...
const UsuariosModule = (function() {
    ...
    function initModalEditarPerfil() {
        $('#modalEditarMiPerfil').on('show.bs.modal', function(event) {
            ...
            const targetRol = $(button).data('rol');//Se agrego para poder bloquear el select de rol si es un super usuario
            ...
            ajaxCall('user', 'save', $(this).serialize())
                .done(r => {
                    if (r.success) {
                        ...
                        setTimeout(() => { window.location.href = BASE_URL + 'index.php?controller=user&action=profile'; }, 900);
                    } else {
...
const EgresosModule = (function() {
        ...
        return ajaxCall('presupuesto', 'getFilteredSubPresupuestos', {}, 'GET')
            ...
            .catch(err => {
                ...
                if (err.responseJSON && err.responseJSON.error) {
                    errorDetail = err.responseJSON.error;
                } else if (err.responseText) {
                    errorDetail = err.responseText.substring(0, 200);
                } else if (err.statusText) {
                    errorDetail = err.statusText;
                }
                
                console.error("Detalle:", errorDetail);
                $selectPres.empty().append(`<option value="">Error: ${errorDetail}</option>`);
                $selectCat.empty().append(`<option value="">Error: ${errorDetail}</option>`);
            });
    }
    ...
    function initEditarMontoEgreso() {
        ...
        $(document).on('click', '#btnGuardarNuevoMontoEgreso', function() {
            ...
            let monto = $('#editMontoEgresoValor').val(); // Lo cambiamos a 'let' para poder modificarlo

        // Se limpian comas y signo de pesos en el navegador antes de validar
        if (monto) {
        ...
        if (monto === '' || isNaN(monto) || parseFloat(monto) < 0) {
            showError('El monto es requerido, debe ser un número válido y no puede ser negativo.');
            return;
        }
/* ======================================================
    Logica anterior que estaba comentada, pero ahora se valida antes de entrar al try-catch
    ======================================================
            // const id = $('#editMontoEgresoId').val();
...
const PresupuestosModule = (function() {
    ...
    // 1. Declaramos el candado FUERA de la función
let guardandoPresupuestoGeneral = false;

function initSubmitPresupuestoGeneral() {
    // 2. Usamos .off() antes de .on() para evitar que el evento se duplique si abres el modal varias veces
    $(document).off('submit', '#formPresupuestoGeneral').on('submit', '#formPresupuestoGeneral', function(e) {
        e.preventDefault();

        // 3. LA MAGIA: Si el candado está cerrado (true), abortamos inmediatamente
        if (guardandoPresupuestoGeneral) {
            console.log("Bloqueado: Ya hay una petición en camino.");
            return; 
        }

        // 4. Cerramos el candado porque apenas vamos a enviar los datos
        guardandoPresupuestoGeneral = true;

        // Intentamos deshabilitar el botón buscando en todo el modal (no solo en el form)
        const $btnSubmit = $('#modalPresupuestoGeneral').find('.btn-primary, button[type="submit"]');
        ...
        ajaxCall('presupuesto', 'save', $(this).serialize())
            .done(r => {
                if (r.success) {
                    ...
                    // Nota: Aquí NO abrimos el candado porque la página se va a recargar y limpiará todo.
                    setTimeout(() => { window.location.reload(); }, 900);
                } else {
                    showError('Error: ' + (r.error || 'Intenta de nuevo.'));
                    // Hubo error lógico, abrimos el candado y rehabilitamos el botón
                    guardandoPresupuestoGeneral = false;
                    $btnSubmit.prop('disabled', false).text('Guardar Presupuesto General');
                }
            })
            .fail(xhr => {
                mostrarError('guardar presupuesto general', xhr);
                // Hubo error de red, abrimos el candado
                guardandoPresupuestoGeneral = false;
                $btnSubmit.prop('disabled', false).text('Guardar Presupuesto General');
            });
    });
...
$(document).on('click', '.btn-close-presupuesto', function(e) {
    ...
    const id = $(this).data('id');
    if (!id) return;
        showConfirm('¿Cerrar este presupuesto? Esta acción bloqueará el uso para nuevos egresos. ¿Desea continuar?')        .then(function(confirmed) {
            if (!confirmed) return;
