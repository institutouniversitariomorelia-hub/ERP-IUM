...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 24-11-2025 a las 20:13:08
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_validar_pagos_parciales` (IN `p_folio_ingreso` INT)   BEGIN
    SELECT 
        i.folio_ingreso,
        i.monto AS total_ingreso,
        COALESCE(SUM(pp.monto), 0) AS total_pagos,
        (i.monto - COALESCE(SUM(pp.monto), 0)) AS diferencia,
        CASE 
            WHEN ABS(i.monto - COALESCE(SUM(pp.monto), 0)) < 0.01 THEN 'CORRECTO'
            ELSE 'ERROR EN MONTOS'
        END AS estatus
    FROM ingresos i
    LEFT JOIN pagos_parciales pp ON i.folio_ingreso = pp.folio_ingreso
    WHERE i.folio_ingreso = p_folio_ingreso
...
--
DELIMITER $$
CREATE TRIGGER `trg_categorias_after_insert` AFTER INSERT ON `categorias` FOR EACH ROW BEGIN
...
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `trg_categorias_after_update_espejo` AFTER UPDATE ON `categorias` FOR EACH ROW BEGIN
...
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `trg_egresos_after_insert_espejo` AFTER INSERT ON `egresos` FOR EACH ROW BEGIN
...
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `trg_egresos_after_update_espejo` AFTER UPDATE ON `egresos` FOR EACH ROW BEGIN
...
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `trg_egresos_before_delete_espejo` BEFORE DELETE ON `egresos` FOR EACH ROW BEGIN
...
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `trg_ingresos_after_insert_espejo` AFTER INSERT ON `ingresos` FOR EACH ROW BEGIN
...
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `trg_ingresos_after_update_espejo` AFTER UPDATE ON `ingresos` FOR EACH ROW BEGIN
...
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `trg_ingresos_before_delete_espejo` BEFORE DELETE ON `ingresos` FOR EACH ROW BEGIN
...
--
-- Disparadores `pagos_parciales`
--
DELIMITER $$
CREATE TRIGGER `trg_pagos_parciales_after_insert` AFTER INSERT ON `pagos_parciales` FOR EACH ROW BEGIN 
    INSERT INTO `erp_ium_espejo`.`pagos_parciales` 
    VALUES (NEW.id_pago_parcial, NEW.folio_ingreso, NEW.metodo_pago, NEW.monto, NEW.orden, NEW.fecha_registro); 
END
$$
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `trg_pagos_parciales_after_update` AFTER UPDATE ON `pagos_parciales` FOR EACH ROW BEGIN 
    UPDATE `erp_ium_espejo`.`pagos_parciales` 
    SET folio_ingreso = NEW.folio_ingreso, metodo_pago = NEW.metodo_pago, monto = NEW.monto, orden = NEW.orden, fecha_registro = NEW.fecha_registro
    WHERE id_pago_parcial = NEW.id_pago_parcial; 
END
$$
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `trg_pagos_parciales_before_delete` BEFORE DELETE ON `pagos_parciales` FOR EACH ROW BEGIN 
    DELETE FROM `erp_ium_espejo`.`pagos_parciales` WHERE id_pago_parcial = OLD.id_pago_parcial; 
END
$$
DELIMITER ;
...
CREATE TRIGGER `trg_presupuestos_after_insert` AFTER INSERT ON `presupuestos` FOR EACH ROW BEGIN
    INSERT INTO `erp_ium_espejo`.`presupuestos` 
    (`id_presupuesto`, `monto_limite`, `nombre`, `fecha`, `id_categoria`, `id_user`, `parent_presupuesto`)
    VALUES 
    (NEW.id_presupuesto, NEW.monto_limite, NEW.nombre, NEW.fecha, NEW.id_categoria, NEW.id_user, NEW.parent_presupuesto);
END
$$
DELIMITER ;
DELIMITER $$
...
CREATE TRIGGER `trg_presupuestos_after_update_espejo` AFTER UPDATE ON `presupuestos` FOR EACH ROW BEGIN
    ...
    SET 
        ...
        id_categoria = NEW.id_categoria, 
        id_user = NEW.id_user,
        parent_presupuesto = NEW.parent_presupuesto
...
--
ALTER TABLE `auditoria`
  MODIFY `id_auditoria` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=397;

--
-- AUTO_INCREMENT de la tabla `categorias`
--
ALTER TABLE `categorias`
  MODIFY `id_categoria` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=247;

--
-- AUTO_INCREMENT de la tabla `egresos`
--
ALTER TABLE `egresos`
  MODIFY `folio_egreso` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=4;

--
-- AUTO_INCREMENT de la tabla `ingresos`
--
ALTER TABLE `ingresos`
  MODIFY `folio_ingreso` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=8;

--
-- AUTO_INCREMENT de la tabla `pagos_parciales`
--
ALTER TABLE `pagos_parciales`
  MODIFY `id_pago_parcial` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=10;
...
--
ALTER TABLE `pagos_parciales`
  ADD CONSTRAINT `fk_pago_parcial_ingreso` FOREIGN KEY (`folio_ingreso`) REFERENCES `ingresos` (`folio_ingreso`) ON DELETE CASCADE ON UPDATE CASCADE;
...
ALTER TABLE `presupuestos`
  ...
  ADD CONSTRAINT `fk_presupuestos_parent` FOREIGN KEY (`parent_presupuesto`) REFERENCES `presupuestos` (`id_presupuesto`) ON DELETE CASCADE ON UPDATE CASCADE,
