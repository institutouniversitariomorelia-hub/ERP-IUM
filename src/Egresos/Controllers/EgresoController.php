...
class EgresoController {
    ...
    public function save() {
        ...
        // --- Limpiar comas y signo de pesos del monto antes de validar ---
        if (isset($data['monto'])) {
            $data['monto'] = str_replace(['$', ','], '', (string)$data['monto']);
        }
         ...
         // Validar categoría sólo si no es reembolso (en reembolso ya se forzó a 224)
         if (!$isReembolso) {
    ...
    public function updateMonto() {
        ...
        $response = ['success' => false]; // La dejamos aquí arriba para que no marque error

        // 1. Limpiamos el monto (le quitamos el signo $ y las comas)
        if ($nuevoMonto !== null) {
            $nuevoMonto = str_replace(['$', ','], '', (string)$nuevoMonto);
        }

        // 2. Validamos el ID del egreso
        if ($folio_egreso_id <= 0) {
            $response['error'] = 'ID de egreso inválido.';
            echo json_encode($response); exit;
        }

        // 3. Validamos el monto (¡Ya limpio de comas!)
        if ($nuevoMonto === null || $nuevoMonto === '' || !is_numeric($nuevoMonto) || $nuevoMonto < 0) {
            $response['error'] = 'El monto no puede ser negativo o contiene un formato inválido.';
            echo json_encode($response); exit;
        }
