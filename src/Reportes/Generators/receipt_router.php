...
if ($tipo === 'egreso') {
    ...
    header("Location: generate_receipt_egreso.php?folio=$folio" . ($reimpresion ? "&reimpresion=1" : ""));
    exit;
}
...
switch ($concepto) {
    case 'Registro Diario':
        header("Location: generate_receipt_ingreso_diario.php?folio=$folio$reimpresionParam");
        break;
    
    case 'Titulaciones':
        header("Location: generate_receipt_ingreso_titulacion.php?folio=$folio$reimpresionParam");
        break;
    
    case 'Inscripciones y Reinscripciones':
        header("Location: generate_receipt_ingreso_inscripcion.php?folio=$folio$reimpresionParam");
        break;
