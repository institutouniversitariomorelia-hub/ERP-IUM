...
class PresupuestoModel {
    ...
    public function getAllPresupuestos() {
        // Seleccionar solo columnas que existen en presupuestos
        $query = "SELECT p.id_presupuesto, p.parent_presupuesto, p.fecha, p.monto_limite, 
                         p.id_categoria,
    ...
    /**
     * NUEVA: Obtiene sub-presupuestos activos SIN p.es_permanente ni p.activo (columnas no existen)
     * @return array Lista limpia de sub-presupuestos
     */
    public function getSubPresupuestosActivos() {
        $query = "SELECT p.id_presupuesto, p.fecha, p.monto_limite, c.nombre AS cat_nombre, 
                         p.id_categoria, p.parent_presupuesto,
        ...
        $result = $this->db->query($query);
        if (!$result) {
            error_log('Error en getSubPresupuestosActivos: ' . $this->db->error);
            return [];
        }
    ...
    public function savePresupuesto($data, $id = null) {
        ...
        // (LÍNEAS 167-169 ELIMINADAS PARA QUE NO REASIGNE EL ID DEL PADRE)
