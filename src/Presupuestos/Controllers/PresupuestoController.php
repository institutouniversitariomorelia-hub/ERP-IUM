...
class PresupuestoController {
    ...
    public function save() {
        ...
        // CORRECCIÓN: Si viene categoría o monto, es un sub-presupuesto. El id_presupuesto es del padre, NO el ID del registro propio.
        // CORRECCIÓN ESTRUCTURAL: Ajustado a la columna real 'parent_presupuesto'
       // ==========================================
        // CORRECCIÓN DEFINITIVA:FLUJO DE PRESUPUESTOS
    ...
    public function getFilteredSubPresupuestos() {
        // Timestamp para evitar cache: 2026-07-07-14:30
        ...
        // QUERY LIMPIA SIN p.es_permanente
        $presupuestos = $this->presupuestoModel->getSubPresupuestosActivos();
        
        $out = [];
        if (!empty($presupuestos) && is_array($presupuestos)) {
            foreach ($presupuestos as $p) {
                $gastado = floatval($p['gastado'] ?? 0);
                $monto = floatval($p['monto_limite'] ?? 0);
    ...
    public function getReembolsoPresupuestos() {
        ...
        $query = "SELECT p.id_presupuesto, p.parent_presupuesto, p.fecha, p.monto_limite, p.id_categoria, c.nombre AS cat_nombre
                  FROM presupuestos p
        ...
        if ($res) {
            while ($row = $res->fetch_assoc()) {
                ...
                $gastado = $this->presupuestoModel->getGastadoEnPresupuesto($idp);
                $disponible = floatval($row['monto_limite'] ?? 0) - floatval($gastado);
